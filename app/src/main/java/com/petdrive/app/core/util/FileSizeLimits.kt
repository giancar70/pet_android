...
/// Mirrors the backend's PetSerializer.ALLOWED_IMAGE_TYPES / DocumentSerializer.
/// ALLOWED_FILE_TYPES -- checked client-side too so a disallowed file is rejected
/// instantly instead of after a wasted upload round-trip. The backend remains the
/// authoritative check either way.
object AllowedFileTypes {
    val IMAGE_TYPES = setOf("image/png", "image/jpeg")
    val DOCUMENT_TYPES = setOf("application/pdf", "image/png", "image/jpeg")

    fun imageTypeMessage() = "La foto solo puede ser PNG, JPG."
    fun documentTypeMessage() = "El documento solo puede ser PDF, PNG, JPG."
}
